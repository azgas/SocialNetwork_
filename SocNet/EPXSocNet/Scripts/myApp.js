...
[
    "$location", "$scope", "$http", "myService", function ($location, $scope, $http, myService) {
        ...
        $scope.host = "https://localhost:44368/",
        $scope.getNetworks = function () {
            $http({
                method: 'GET',
                url: $scope.host + "api/network/GetListOfNetworks"
            })
                ...
                .error(function () {
                    alert("Error: couldn't get list of networks");
                });
        }
        $scope.getNetworks();
        ...
        $scope.getVertex = function () {
            $http({
                method: 'GET',
                url: $scope.host + "api/network/GetListOfVertices/" + $scope.selectedNetwork.id + "?date=" + $scope.selectedDate
            })
                .success(function (data) {
                    $scope.vertices = data;
                    var all = { "name": "Show all", "id": 0 };
                    $scope.vertices.unshift(all);
                    $scope.selectedVertex = $scope.vertices[1];
                    $scope.refresh();
                })
                .error(function () {
                    alert("Error: couldn't get list of vertices");
                });
        };
...
[
    "$location", "$http", "$scope", "myService", function ($location, $http, $scope, myService) {
        ...
        $scope.nextindex = function () {
            $http.get($scope.host + "api/network/GetNetwork/" + $scope.selectedNetwork.id + "?date=" + $scope.selectedDate + "&incl=" + $scope.checkInclude)
                .success(function (data) {
        ...
        $scope.$watch("ref",
            function () {
                $http.get($scope.host + "api/network/GetFactors/" + $scope.selectedNetwork.id + "?date=" + $scope.selectedDate)
                    .success(function (data) {
                        $scope.factors = data;
                        if ($scope.factors.status !== "Factors weren't estimated yet") {
        ...
        $scope.countFactors = function () {
            if ($scope.checkIndegree && $scope.checkOutdegree && $scope.checkBetweenness && $scope.checkCloseness && $scope.checkInfluence && $scope.checkDensity)
                $http.get($scope.host + "api/CountFactors/CountAll/" +
                ...
                $http.get($scope.host + "api/CountFactors/IndegreeCentrality/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
                ...
                $http.get($scope.host + "api/CountFactors/OutdegreeCentrality/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
                ...
                $http.get($scope.host + "api/CountFactors/BetweennessCentrality/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
                ...
                $http.get($scope.host + "api/CountFactors/ClosenessCentrality/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
                ...
                $http.get($scope.host + "api/CountFactors/InfluenceRange/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
                ...
                $http.get($scope.host + "api/CountFactors/Density/" +
                        $scope.selectedNetwork.id +
                        "?date=" +
                        $scope.selectedDate)
                    .success(function () {
            ...
            link: function ($scope) {
                $scope.$watch("ref",
                    function () {
                        ...
                        d3.json("https://localhost:44368/api/network/GetNetwork/" + $scope.selectedNetwork.id + "?date=" + $scope.selectedDate + "&vertid=" + $scope.selectedVertex.id + "&incl=" + +$scope.checkInclude,
